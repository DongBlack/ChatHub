        ...
        <template v-for="item in activeRoom.messages">
          <div class="message-content-message" :key="item.userId + item.time" :class="{ 'text-right': item.userId === user.userId }">
            ...
            <div>
              ...
              <div class="message-content-image" v-if="item.messageType.split('*')[0] === 'file'" :style="getImageStyle(item.content)">
                <viewer class="message-file">
                  <img class="message-file-img" src="~@/assets/photo.png" alt="" />
                  <div class="file-info">
                    <div class="file-name">{{ item.content.split('$')[item.content.split('$').length - 1] }}</div>
                    <div class="download-div"><span class="download-btn"><a :href="'api/static/' + item.content" download>下载</a></span></div>
                  </div>
                </viewer>
              </div>
            </div>
